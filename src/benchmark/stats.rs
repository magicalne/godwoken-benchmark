...
use super::msg::{ApiStatus, StatsReqMsg};
...
impl StatsActor {
    ...
    fn handle_tx_status(&mut self, status: TxStatus) {
        match status {
            ...
            TxStatus::Committed(_) => self.tx_stats.committed += 1,
            TxStatus::Failure => self.tx_stats.failure += 1,
            TxStatus::Timeout(_) => self.tx_stats.timeout += 1,
        };
...
impl ApiStatsHandler {
    ...
    fn stats(&self) -> ApiStats {
        ...
        let success_rate = self.success as f32 / elasped_secs;
        let failure_rate = self.failure as f32 / elasped_secs;
        ApiStats {
            min: self.min,
            max: self.max,
            avg,
