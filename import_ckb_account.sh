...
for i in $(seq $count); do
        ...
        if [ ! -f $account_path ]; then
                ...
                output=$(yes qwe | ckb-cli --url $1 account import --privkey-path $pk_path)
                read pk < $pk_path
                echo $output
...
for i in $(seq $count); do
        ...
        ckb-cli --url $1 wallet transfer --privkey-path issue_pk --to-address $(jq -r '.testnet' $account_path) --capacity $amount --tx-fee 0.00001
        while true; do
                capacity=$(ckb-cli --url $1 wallet get-capacity --address $addr)
